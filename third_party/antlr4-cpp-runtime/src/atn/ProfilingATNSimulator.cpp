...
ProfilingATNSimulator::ProfilingATNSimulator(Parser* parser)
    : ParserATNSimulator(
          parser,
          parser->getInterpreter<ParserATNSimulator>()->atn,
...
bool ProfilingATNSimulator::evalSemanticContext(
    Ref<const SemanticContext> const& pred,
    ParserRuleContext* parserCallStack,
    size_t alt,
    bool fullCtx) {
  bool result = ParserATNSimulator::evalSemanticContext(pred, parserCallStack,
                                                        alt, fullCtx);
  if (!(std::dynamic_pointer_cast<const SemanticContext::PrecedencePredicate>(
            pred) != nullptr)) {
...
void ProfilingATNSimulator::reportAmbiguity(DFA& dfa,
                                            DFAState* D,
                                            size_t startIndex,
                                            size_t stopIndex,
                                            bool exact,
